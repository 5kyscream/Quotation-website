...
### Added
- **Custom Color Picker**: Replaced native OS color pickers with a custom UI component featuring standard neutral/brand palettes and an integrated custom hex selector button.
- **Intelligent PDF Theming**: Implemented dynamic luminance calculations in the PDF engine. Text colors, section headings, and the company logo now automatically invert (black/white) to ensure perfect contrast against custom background and card colors.
...
### Changed
- **Theme Panel Layout**: Moved the global theme settings from an inline block to a persistent, floating vertical dock on the right side of the screen, maximizing vertical space for the live PDF preview.
